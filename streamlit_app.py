...
with tab2:
    # --- 파일 업로드를 일지 상단에 배치 (오빠 요청사항) ---
    st.subheader("📉 선물 매매 기록 (CSV 업로드)")
    uploaded_file = st.file_uploader("엑셀(CSV) 파일을 여기에 드래그하거나 클릭하세요", type=["csv"])
    ...
    if uploaded_file:
        try:
            ...
            # 월별 수익 파싱 (2~3행)
            try:
                m_header = [f"{i+1}월" for i in range(12)]
